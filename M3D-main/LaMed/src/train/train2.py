...
import torch
import transformers
from transformers import AutoTokenizer, LlamaForCausalLM, TrainerCallback
...
class DataArguments:
    ...
    train_dataset: str = field(
        default="auto",
        metadata={"help": "Training dataset: auto, cap, text, uni."},
    )

    # caption data
    cap_data_path: str = field(default="./Data/data/my_dataset.json", metadata={"help": "Path to caption data."})
...
class EvalLogCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if eval_loss is not None:
            rank0_print(f"[eval] step={state.global_step} eval_loss={eval_loss:.6f}")
        else:
            rank0_print(f"[eval] step={state.global_step} metrics={metrics}")
...
def main():
    ...
    if data_args.train_dataset == "auto":
        if model_args.tune_mm_mlp_adapter:
            if os.path.exists(data_args.vqa_data_train_path):
                train_dataset = TextDatasets(data_args, tokenizer, mode='train')
                train_dataset_name = "TextDatasets"
            else:
                logger.warning(
                    "vqa_data_train_path not found; falling back to CapDataset. "
                    "Set --train_dataset text to force TextDatasets."
                )
                train_dataset = CapDataset(data_args, tokenizer, mode='train')
                train_dataset_name = "CapDataset"
        else:
            train_dataset = UniDatasets(data_args, tokenizer, mode='train')
            train_dataset_name = "UniDatasets"
    elif data_args.train_dataset == "cap":
        train_dataset = CapDataset(data_args, tokenizer, mode='train')
        train_dataset_name = "CapDataset"
    elif data_args.train_dataset == "text":
        train_dataset = TextDatasets(data_args, tokenizer, mode='train')
        train_dataset_name = "TextDatasets"
    elif data_args.train_dataset == "uni":
        train_dataset = UniDatasets(data_args, tokenizer, mode='train')
        train_dataset_name = "UniDatasets"
    else:
        raise ValueError(f"Unknown train_dataset: {data_args.train_dataset}")

    eval_dataset = CapDataset(data_args, tokenizer, mode='validation')
    data_collator = DataCollator(data_args.seg_enable)
    rank0_print(f"Train dataset: {train_dataset_name}, size: {len(train_dataset)}")
    rank0_print(f"Eval dataset: CapDataset, size: {len(eval_dataset)}")
    if len(eval_dataset) == 0:
        logger.warning("Eval dataset length is 0; eval_loss will be missing. Check cap_data_path and data_root.")
    ...
    trainer.add_callback(EvalLogCallback())
