...
class LamedMetaModel:
    ...
    def initialize_vision_modules(self, model_args):
        ...
        if model_args.pretrain_vision_model is not None:
            ...
            load_info = self.vision_tower.vision_tower.load_state_dict(vision_model_weights, strict=False)
            if load_info.missing_keys:
                logger.warning(
                    "Vision checkpoint missing %d keys (showing first 5): %s",
                    len(load_info.missing_keys),
                    load_info.missing_keys[:5],
                )
            if load_info.unexpected_keys:
                logger.warning(
                    "Vision checkpoint has %d unexpected keys (showing first 5): %s",
                    len(load_info.unexpected_keys),
                    load_info.unexpected_keys[:5],
                )
